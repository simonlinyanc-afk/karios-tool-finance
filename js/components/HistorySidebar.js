const HistorySidebar = ({ isOpen, onClose, onRestore }) => {
    const { X, RefreshCw, Briefcase, Trash2, Download } = window; // Added Download
                                    ...
                                    <div key={record.id} className="group relative bg-[#252525] border border-[#333] rounded-lg p-5 hover:border-yellow-500 hover:shadow-lg hover:shadow-yellow-500/10 transition-all text-left">
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex flex-col flex-1">
                                                <span className="text-sm font-bold text-gray-200 font-cn whitespace-normal break-words leading-relaxed" title={displayTitle}>
                                                    {displayTitle}
                                                </span>
                                        ...
                                        <div className="mb-4 bg-[#1f1f1f] p-3 rounded border border-[#2a2a2a]">
                                        ...
                                        <div className="flex gap-2 pt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                onClick={() => {
                                                    if (window.exportToExcel) {
                                                        window.exportToExcel(items, columns.length ? columns : null, info);
                                                    } else {
                                                        alert('导出模块未加载');
                                                    }
                                                }}
                                            ...
                                            >
                                                <Download size={14} />
                                                <span className="font-en">Excel</span>
                                            </button>
                                            <button
